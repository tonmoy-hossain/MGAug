...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.skip_connect = kwargs.get('skip_connect', False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #### Gaussian Mixture and Variational Sampling Block
        ...
        # total loss
        loss_total = 1 * loss_gauss + 1 * loss_cat + 1 * loss_rec
        
        x = data_recon.view(-1, 32, 8, 8)
        #### Gaussian Mixture and Variational Sampling Block
        ...
        # print("EPDiff Velocity shape: ", v.shape)
        u_seq, ui_seq, v_seq = self.MEpdiff.my_expmap_u2phi(m, num_steps=self.TSteps)
        # u_seq, v_seq, m_seq, ui_seq = \
        #     self.MEpdiff.my_expmap( m, num_steps=self.TSteps)
        u = u_seq[-1]
        ui = ui_seq[-1]
        # print(src.shape, u.shape)
        Sdef = lm.interp(src, u)

        # print("Deformed Image shape: ", Sdef.shape)
        
        return Sdef, encoder_output, v, m, u, loss_total 
...
class DiffeoDense(LoadableModel):
    ...
    def forward(self, source, target, registration=False):
        ...
        x,latent_f, vae_loss = self.unet_model(x) #vae_loss
        # print("UNet Reg output shape: ", x.shape)

        # transform into flow field
        x = self.flow(x).permute(0, 2, 3, 1)

        # print("After flow and permute shape: ", x.shape)
        ...
        if not registration:
            ...
            return Sdef, v, latent_f, vae_loss, m, ui, 
            # return y_source, pos_flow, latent_f, vae_loss
